...
async def on_order_status_changed(bot, event):
    ...
    logger.info(
        "Apple TopUp: ORDER_STATUS_CHANGED order=%s status=%s",
        order.id,
        order.status
    )
    ...
    if order.status == OrderStatuses.CLOSED:
        ...
        if existing:
            logger.info(
                "🍎 Apple TopUp: заказ %s закрыт в FunPay; "
                "состояние Apple TopUp не изменяем — ждём !оформил или возврат.",
                order.id
            )
...
async def on_new_message(bot, event):
    ...
    if text_lower == "!оформил":
        ...
        logger.info(
            "🍎 Apple TopUp: покупатель подтвердил оформление "
            "заказа %s",
            order_id
        )
