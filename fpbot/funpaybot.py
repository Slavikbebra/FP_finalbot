...
class FunPayBot:
    ...
    def _is_active_apple_topup_chat(self, username, chat_id):
        """Определяет, относится ли текущий чат к активному Apple TopUp-заказу."""
        try:
            from core.modules import get_modules

            apple_module = next(
                (module for module in get_modules() if module.meta.prefix == "apple_topup"),
                None,
            )
            if not apple_module or not apple_module.enabled:
                return False

            from modules.apple_topup import storage as apple_storage
            return apple_storage.find_active_order(username, chat_id) is not None
        except Exception:
            return False
    ...
    async def _on_new_message(self, event: NewMessageEvent):
        ...
        if str(event.message.text).lower() in ("!продавец", "!seller"):
            if not self._is_active_apple_topup_chat(
                event.message.author,
                getattr(event.message, "chat_id", None),
            ):
                asyncio.run_coroutine_threadsafe(
